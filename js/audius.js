...
export async function searchTracks(query, limit = 20) {
  const data = await apiFetch(`/tracks/search?query=${encodeURIComponent(query)}&limit=${limit}`);
  return data.data || [];
}

export async function getTrending(genre = null, time = 'week', limit = 20) {
  let path = `/tracks/trending?time=${time}&limit=${limit}`;
